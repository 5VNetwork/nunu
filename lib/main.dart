...
import 'package:nunu/app/privacy.dart';
...
import 'package:nunu/app/home.dart';
import 'package:nunu/app/top_bar.dart';
import 'package:nunu/app/welcome.dart';
...
import 'package:nunu/utils/geodata.dart';
import 'package:nunu/utils/windows_host_api.g.dart';
import 'package:nunu/auth/auth_bloc.dart';
import 'package:nunu/common/bloc_observer.dart';
import 'package:nunu/common/common.dart';
import 'package:flutter/cupertino.dart';
import 'package:nunu/utils/root.dart';
import 'package:nunu/utils/upload_log.dart';
...
import 'package:nunu/firebase_options.dart';
...
void main() async {
  ...
  final authProvider = SupabaseAuth(
    ...
    loginCallbackUrl: 'nunu://login-callback',
  );
  if (kDebugMode) {
...
Future<String> assetName() async {
  if (Platform.isAndroid) {
    return 'nunu-arm64-v8a.apk.zip';
  } else if (Platform.isWindows) {
    // final ar = await arch();
    return 'NunuInstaller.exe';
  } else if (isRpm()) {
    final ar = await arch();
    if (ar.contains('arm64')) {
      return 'nunu-arm64.rpm';
    }
    return 'nunu-x64.rpm';
  } else {
    final ar = await arch();
    if (ar.contains('arm64')) {
      return 'nunu-arm64.deb';
    }
    return 'nunu-x64.deb';
  }
}
