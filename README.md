# <img src="assets/icons/icon.svg" alt="Nunu Icon" width="28" height="28" align="absmiddle"/> Nunu · 努努加速器

**免费 VPN / 梯子** — 永久免费、不限流量的网络加速工具。一键连接，科学上网更简单。

Nunu（努努加速器）是一款面向手机的 **免费 VPN**，帮助你稳定访问网络、保护隐私与上网自由。无论你是在找「梯子」「翻墙」工具，还是日常要用的 **VPN**，努努都承诺：**完全永久免费，不限流量**。

官网：[www.nunu.monster](https://www.nunu.monster) · [备用官网](https://www.nunuvpn.com/zh)

## 为什么选择努努（免费 VPN / 梯子）

| 特性 | 说明 |
| --- | --- |
| 永久免费 | 不设付费墙，不会突然收费 |
| 不限流量 | 浏览、视频、学习场景均可使用 |
| 一键连接 | 适合想要简单「科学上网」体验的用户 |
| 隐私保护 | 标准加密传输，不记录浏览历史 |

市面上不少「免费 VPN」会限速、限流量或诱导开通付费套餐。努努加速器定位为长期可用的 **免费梯子**：用广告维持运营，让你安心使用网络加速，而不是担心配额用完。

## 适合谁用

- 搜索「**免费 VPN**」「**梯子**」却不想每月续费的人
- 需要稳定 **VPN** / **科学上网** 体验的学生与日常用户
- 希望用更简单方式处理 **翻墙** / 网络受限场景的用户

## 使用须知

- **免费使用**，永久不向用户收费
- **禁止种子下载**：种子流量会直接发出，不经过 VPN 隧道

## About LICENSE

We retain all rights to our source code and no one may reproduce, distribute, or
create derivative works from our work.
